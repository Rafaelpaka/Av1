    ...
    public class CadastroJogo
    {
        ...
        public void Excluir()
        {
            ...
            if (int.TryParse(Console.ReadLine(), out int idexcluir))
            {
                ...
                Jogo JogoExcluir = jogos.FirstOrDefault(j => j.Id == idexcluir);
        ...
        public void Cadastrar()
        {
            ...
            Jogo novoJogo = new();
                ...
                while (!int.TryParse(Console.ReadLine(), out anoJogo) || anoJogo < 1950 || anoJogo > DateTime.Now.Year)
                {
                    Console.WriteLine($"Ano inválido. não é possivel registrar jogos anteiores a 1972 e posteriores à {DateTime.Now.Year}.");
                    Console.Write("Ano do jogo: ");
            }
            novoJogo.Ano = anoJogo;


            Jogo jogoDuplicado = jogos.FirstOrDefault(j => j.Nome.ToLower() == novoJogo.Nome.ToLower());

            if (jogoDuplicado != null)
            {
                Console.WriteLine($"\nJá existe um jogo chamado '{novoJogo.Nome}.");
                Console.Write("Deseja mesmo cadastrar um jogo duplicado? (S/N): ");
                ...
                Console.WriteLine("Continuando com o cadastro do jogo duplicado.");
            }

            novoJogo.Id = jogos.Any() ? jogos.Max(j => j.Id) + 1 : 1;

            jogos.Add(novoJogo);

                string novoJson = JsonSerializer.Serialize(jogos, new JsonSerializerOptions { WriteIndented = true }); // [AV1-3]
            File.WriteAllText(caminhoArquivo, novoJson);
